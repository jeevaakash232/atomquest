...
import { User, Lock, Eye, EyeOff, Loader2, Mail, Building2, ChevronDown, ArrowRight } from "lucide-react";
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/signup", {
        ...
        body: JSON.stringify({ name, email, password, role, department: department || role }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error ?? "Signup failed"); return; }
      clearSessionCache();
      router.push(data.redirectTo ?? ROLE_REDIRECTS[role]);
      router.refresh();
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        <img src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=900&q=85"
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-12">
          ...
          <div>
            <h2 className="font-serif text-4xl font-bold text-white mb-3 leading-tight">
              Join 500+ Teams<br />Achieving More
            </h2>
      ...
      {/* Right — form */}
      <div className="flex-1 flex items-center justify-center p-8 bg-[#faf8f5] overflow-y-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
          ...
          <div className="mb-7">
            ...
            <h1 className="font-serif text-3xl font-bold text-gray-900 mb-1">Create account</h1>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Smith"
                  className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 outline-none focus:border-[#c9a84c] focus:ring-2 focus:ring-[#c9a84c]/20 placeholder:text-gray-400 transition-all" required />
              </div>
            </div>

            {/* Email */}
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Work Email</label>
              <div className="relative">
              ...
              <div>
                ...
                <div className="relative">
                  <select value={role} onChange={(e) => setRole(e.target.value as Role)}
                    className="w-full pl-3 pr-8 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 outline-none focus:border-[#c9a84c] focus:ring-2 focus:ring-[#c9a84c]/20 appearance-none cursor-pointer transition-all">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={department} onChange={(e) => setDepartment(e.target.value)} placeholder="Engineering"
                    className="w-full pl-9 pr-3 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 outline-none focus:border-[#c9a84c] focus:ring-2 focus:ring-[#c9a84c]/20 placeholder:text-gray-400 transition-all" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPwd ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 6 characters"
                  className="w-full pl-10 pr-10 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 outline-none focus:border-[#c9a84c] focus:ring-2 focus:ring-[#c9a84c]/20 placeholder:text-gray-400 transition-all" required />
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showConfirm ? "text" : "password"} value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="Re-enter password"
                  className={`w-full pl-10 pr-10 py-3 bg-white border rounded-xl text-sm text-gray-900 outline-none focus:ring-2 placeholder:text-gray-400 transition-all ${confirm && password !== confirm ? "border-red-300 focus:border-red-400 focus:ring-red-100" : "border-gray-200 focus:border-[#c9a84c] focus:ring-[#c9a84c]/20"}`} required />
              ...
              {confirm && password !== confirm && <p className="text-[10px] text-red-500 mt-1">Passwords do not match</p>}
            </div>
          ...
          <div className="mt-5">
            ...
            <button onClick={() => window.location.href = "/api/auth/google"}
              className="w-full flex items-center justify-center gap-2.5 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-all shadow-sm">
